...
void TriMesh::generateDisk(int num_division, float radius)
{
	...
	vertex_colors.push_back(glm::vec3(0.0f, 0.0f, 1.0f));
	...
	for (int i = 0; i < num_samples; i++)
	{
		...
		vertex_normals.push_back(glm::vec3(0.0f, 0.0f, 1.0f));
		vertex_colors.push_back(glm::vec3(x, y, 0.0f));
	}

	// 生成三角面片，从中心向外辐射
	for (int i = 0; i < num_samples; i++)
	{
		int next_i = (i + 1) % num_samples;

		// 每个三角形：中心顶点 -> 当前边缘顶点 -> 下一个边缘顶点
		faces.push_back(vec3i(0, i + 1, next_i + 1));

		// 计算纹理坐标（极坐标到笛卡尔坐标的映射）
		float current_angle = i * step;
		float next_angle = next_i * step;

		// 中心点的纹理坐标
		vertex_textures.push_back(glm::vec2(0.5f, 0.5f));
		// 当前边缘点的纹理坐标
		vertex_textures.push_back(glm::vec2(0.5f + 0.5f * cos(current_angle),
			0.5f + 0.5f * sin(current_angle)));
		// 下一个边缘点的纹理坐标
		vertex_textures.push_back(glm::vec2(0.5f + 0.5f * cos(next_angle),
			0.5f + 0.5f * sin(next_angle)));
...
void TriMesh::generateCone(int num_division, float radius, float height)
{
	...
	// 添加圆锥顶点（顶部）
	vertex_positions.push_back(glm::vec3(0.0f, 0.0f, height));
	vertex_normals.push_back(glm::vec3(0.0f, 0.0f, 1.0f));
	vertex_colors.push_back(glm::vec3(1.0f, 0.0f, 0.0f));
	...
	for (int i = 0; i < num_samples; i++)
	{
		...
		// 计算侧面法向量
		glm::vec3 normal = normalize(glm::vec3(x, y, radius * radius / height));
		vertex_normals.push_back(normal);
		vertex_colors.push_back(normalize(glm::vec3(x, y, 0.0f)));
	}
	...
	for (int i = 0; i < num_samples; i++)
	{
		...
		// 侧面三角形：顶部 -> 当前边缘点 -> 下一个边缘点
		faces.push_back(vec3i(top_idx, bottom_start + i, bottom_start + next_i));

		// 侧面纹理坐标
		vertex_textures.push_back(glm::vec2(0.5f, 1.0f)); // 顶部

		float u1 = static_cast<float>(i) / num_samples;
		vertex_textures.push_back(glm::vec2(u1, 0.0f));

		float u2 = static_cast<float>(next_i) / num_samples;
		vertex_textures.push_back(glm::vec2(u2, 0.0f));
...
void TriMesh::readObj(const std::string& filename)
{
	...
	while (std::getline(fin, line))
	{
		...
		// faces normal_index texture_index
		else if (type == "f") {
			// 解析每个顶点的v/vt/vn数据
